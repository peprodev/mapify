...
class Options {
	...
	/**
	 * Option schema: key => [type, default]. Extensions add keys with the mapify_options_schema filter.
	 */
	public static function schema() {
		return apply_filters(
			'mapify_options_schema',
			array(
		...
		register_setting(
			...
			array(
				...
				'show_in_rest'      => array(
					'schema' => array(
						'type'                 => 'object',
						'properties'           => apply_filters( 'mapify_options_rest_properties', $properties ),
	...
	public static function sanitize( $value ) {
		...
		foreach ( self::schema() as $key => $def ) {
			...
			switch ( $def[0] ) {
				...
				case 'integer':
					...
					break;
				case 'array':
					$out[ $key ] = is_array( $raw ) ? $raw : array();
					break;
				default:
		...
		$out = apply_filters( 'mapify_sanitize_options', $out, $value );
		// Keep settings this edition does not know about, so switching editions loses nothing.
		$saved = get_option( self::KEY, array() );
		if ( is_array( $saved ) ) {
			$out = array_merge( array_diff_key( $saved, $out ), $out );
		}
		return $out;
	}
}
